...
* Script must be accessible/readable to Python developers with no experience to Nmap or networking

## How to install/use this project

1. Install the python-nmap module using pip.
> `pip install python-nmap`
2. Install the netifaces module using pip.
> `pip install netifaces`
3. Install Nmap onto your machine. Nmap **MUST** be installed on your machine as the python-nmap module interacts with the application. 
> To download Nmap onto your machine, follow the documentation on [Nmap's website](https://nmap.org/download) for your system.
4. Download `autoNetworkScanner.py` into your desired directory on your machine.
5. Run the script with administrator/root privileges
> For Linux and macOS, write `sudo python autoNetworkScanner.py` into your terminal.

> For Windows, use the shortcut `Windows Key + X` and then click `A` to open a terminal as administrator
...
Since this was just a personal project to reinforce Python syntax for myself and learn about networking modules in Python, I strongly encourage you to read the code and the comments so that you may understand and change it as you want. I suggest messing with the `arguments` parameter for the `nmap.scan()` function if you wish to learn other ways Nmap can scan networks. 
